...
export class Station extends TypedEmitter<StationEvents> {
    ...
    protected constructor(
        ...
        this.p2pSession.on(
            "download started",
            (channel: number, metadata: StreamMetadata, videoStream: PassThrough, audioStream: PassThrough) =>
            ...
            if (message.event_type === CusPushEvent.MODE_SWITCH && message.station_sn === this.getSerial()) {
                ...
                try {
                    if (message.station_guard_mode !== undefined) {
                        ...
                        this.updateProperty(PropertyName.StationPersonName, !isEmpty(message.user_name) ? message.user_name! : "Unknown");

                        this.eventTimeouts.set(DeviceEvent.Other, setTimeout(async () => {
                            this.updateProperty(PropertyName.StationPersonName, "");
                            this.eventTimeouts.delete(DeviceEvent.Other);
            ...
            this.p2pSession.sendCommandWithStringPayload(
                {
                    ...
                    value: JSON.stringify({
                        ...
                        payload: {
                            filepath: path,
                            key: rsa_key?.exportPublicComponents()?.n
                                ? Array.from(rsa_key.exportPublicComponents()!.n.subarray(1))
            ...
            this.p2pSession.sendCommandWithStringPayload(
                {
                    ...
                    value: JSON.stringify({
                        ...
                        data: {
                            ...
                            camera_type: 0,
                             "encryptkey": rsa_key?.exportPublicComponents()?.n
                            ? Array.from(rsa_key.exportPublicComponents()!.n.subarray(1))
                                .map(b => b.toString(16).padStart(2, "0"))
                                .join("")
                            : undefined,
                ...
                this.p2pSession.sendCommandWithInt(
                    {
                        ...
                        value: device.getChannel(),
                        strValue: rsa_key?.exportPublicComponents()?.n
                            ? Array.from(rsa_key.exportPublicComponents()!.n.subarray(1))
                                .map(b => b.toString(16).padStart(2, "0"))
                                .join("")
                            : undefined,
